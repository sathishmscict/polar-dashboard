 ...
 *
 * <p>The LicenseChecker is configured via a {@link Policy} which contains the logic to determine
 ...
 *
 * <p>Must also provide the Base64-encoded RSA public key associated with your developer account.
...
public class LicenseChecker implements ServiceConnection {
   ...
   *
   * <p>NOTE: This call uses a trivially obfuscated string (base64-encoded). For best security, we
   * recommend obfuscating the string that is passed into bindService using another method of your
   * own devising.
   *
   * <p>source string: "com.android.vending.licensing.ILicensingService"
   *
   * <p>
   *
   * @param callback
    ...
    while ((validator = mPendingChecks.poll()) != null) {
      try {
        ...
        mService.checkLicense(
            validator.getNonce(), validator.getPackageName(), new ResultListener(validator));
  ...
  private class ResultListener extends ILicenseResultListener.Stub {
    ...
    public void verifyLicense(
        final int responseCode, final String signedData, final String signature) {
      mHandler.post(
          new Runnable() {
            public void run() {
  ...
  /**
   * Inform the library that the context is about to be destroyed, so that any open connections can
   * be cleaned up.
   *
   * <p>Failure to call this method can result in a crash under certain circumstances, such as
   * during screen rotation if an Activity requests the license check or when the user exits the
   * application.
   */
