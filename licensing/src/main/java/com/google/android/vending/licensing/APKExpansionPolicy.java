 ...
 * period, error retry period, and error retry count.
 *
 * <p>These values will vary based on the the way the application is configured in the Android
 * Market publishing console, such as whether the application is marked as free or is within its
 * refund period, as well as how often an application is checking with the licensing service.
 *
 * <p>Developers who need more fine grained control over their application's licensing policy should
...
public class APKExpansionPolicy implements Policy {
  ...
  public APKExpansionPolicy(Context context, Obfuscator obfuscator) {
    ...
    mValidityTimestamp =
        Long.parseLong(mPreferences.getString(PREF_VALIDITY_TIMESTAMP, DEFAULT_VALIDITY_TIMESTAMP));
   ...
   *
   * <p>This data will be used for computing future policy decisions. The following parameters are
   * processed:
   *
   * <ul>
  ...
  public void processServerResponse(
      int response, com.google.android.vending.licensing.ResponseData rawData) {
  ...
  /**
   * Set the last validity timestamp (VT) received from the server and add to preferences. You must
   * manually call PreferenceObfuscator.commit() to commit these changes to disk.
   *
   ...
   *   <li>a RETRY response was received in the last minute, and we are under the RETRY count or in
   *       the RETRY period.
   * </ol>
   */
  ...
  private Map<String, String> decodeExtras(String extras) {
    ...
    try {
      ...
      List<Item> extraList = URLUtils.parse(rawExtras, "UTF-8");
      for (Item item : extraList) {
